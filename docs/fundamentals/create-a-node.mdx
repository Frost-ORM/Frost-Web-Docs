...
:::info
Since you don't have to define a constructor, then Typescript will throw an error because it cannot detect the intialization of the object properties. 
So you'll have to either use `?` for optional parameters or `!` for non-optional parameters to let Typescript know that they'll be initiallized 
:::
<>{createClass}</>
...
### Available methods
Since the Entity extends FrostObject it inherits a few usefull [methods](/api/classes/FrostObject#methods) from it. Please check them out they might be of help to you.
<!-- :::warning
...
- Annotate the class with the [FrostNode](/api/decorators/FrostNode) decorator. Pass the Corresponding Entity to the deocrator.
